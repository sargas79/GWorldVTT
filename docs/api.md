...
- **Hooks**, each called with a mutable context:
  ...
  - `gworld.vehicleDr` (since 1.79.0): before a vehicle's DR meets a shot
    ...
    its face. Since 1.88.0 a location may give its own faces too:
    ...
    `vehicleFaceDr`, `vehicleLocationDr` (since 1.88.0), `vehicleDrAt`,
    `vehiclePenetration`, `vehicleDrLabel`, `passesThrough` and
    `aimableLocations`.

    Two more of the vehicle's statistics changed shape in 1.88.0, both
