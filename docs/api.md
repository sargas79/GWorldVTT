...
- **Crippled parts** (since 1.114.0; Campaigns p. 422):
  ...
  - `actors.treatCrippled(actor, idOrLocation, { treatedAtTl })` (since
    ...
    `treatedAtTl` and, for a lasting one, `roll` (the 1d); for a part
    recorded before 1.155.0 the die is read back from its months. It
...
- **A linked line's own area** (since 1.155.0; Campaigns p. 381). A linked
  ...
  isn't rolled for; the user is told who was left out. A damage line's card
...
- With `apply: true` it also leaves the result on the victim as timed
  ...
  also ends after 1 and 2 seconds of world time. Blindness is
