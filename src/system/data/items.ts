...
export class TraitData extends foundry.abstract.TypeDataModel {
  ...
  static override defineSchema() {
    return {
      ...
      /**
       * True for an advantage that can be learned as if it were a skill
       * (Learnable Advantages, Characters p. 294), which the Study tool then
       * offers. The Basic Set's are flagged in the compendium; a module flags
       * its own book's the same way (since API 1.146.0).
       */
      learnable: new fields.BooleanField({ required: true, initial: false }),
      /**
