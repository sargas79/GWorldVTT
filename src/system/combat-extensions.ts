...
export const COMBAT_HOOKS = Object.freeze({
  ...
  /**
   * Before a blow's DR is added up (since 1.48.0): `{ actor, item, mode, hitLocation, damageType, basicDamage, ignoresDr, arc, preview, lines }`,
   * the lines mutable (`ignoresDr` since 1.55.0, `arc` since 1.56.0, natural DR as lines since 1.98.0). Since 1.140.0 it also fires
   * with `preview` true for the sheet's DR by location, with no blow behind it.
   */
