...
function criticalRoll(table: CriticalTable) {
  return {
    formula: "3d6",
    rowFor: (total: number) => String(criticalEntry(table, total).rolls[0]),
  };
}
...
const THROWN_BANDS: ReadonlyArray<[number, string]> = [
  ...
  [2, "2×BL"],
  [4, "4×BL"],
  [MAX_THROWABLE_MULTIPLE, `${MAX_THROWABLE_MULTIPLE}×BL`],
];
    ...
    build: ({ t }) => {
      ...
      for (let skill = 3; skill <= 18; skill += 1) {
        ...
        chances.push({
          cells: [
            String(skill),
            percent(odds.success),
            percent(odds.criticalSuccess),
            percent(odds.criticalFailure),
          ],
        });
      }
      return {
        parts: [
          {
            ...
            content: {
              kind: "table",
              columns: [t(`${K}.Column.Score`), t(`${K}.Column.Level`)],
              rows: levels,
            },
          },
          {
      ...
      parts: [
        {
          content: {
            ...
            rows: THROWN_BANDS.map(([ratio, label]) => ({
              cells: [
                t(`${K}.UpTo`, { value: label }),
                thrownDamageText(thrownDamagePerDie(ratio, 1) ?? 0, t),
              ],
            })),
          },
        },
      ...
      parts: [
        {
          content: {
            ...
            rows: THROWING_DISTANCE_TABLE.map(([ratio, modifier]) => ({
              cells: [String(ratio), `×${modifier}`],
            })),
          },
        },
      ],
        ...
        parts: [
          {
            ...
            content: {
              ...
              items: [
                {
                  term: t(partKey("coverDr", "Exposed")),
                  text: t(partKey("coverDr", "ExposedText"), { value: signed(exposed.modifier) }),
                },
                {
                  term: t(partKey("coverDr", "Random")),
                  text: t(partKey("coverDr", "RandomText"), { from: random.strikesCoverOn ?? 4 }),
                },
                {
                  term: t(partKey("coverDr", "Through")),
                  text: t(partKey("coverDr", "ThroughText"), {
                    value: signed(SHOOT_THROUGH_PENALTY),
          ...
          },
          {
            ...
            content: {
              ...
              rows: WALLS.map((wall) => ({
                cells: [wall.name, `${wall.dr}${wall.wearsAway ? "*" : ""}`, String(wall.hp)],
              })),
            },
