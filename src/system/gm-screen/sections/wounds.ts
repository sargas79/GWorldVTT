...
import type { BuildContext, GmPart, GmRow, GmSectionContent, GmSectionDef } from "../types.js";
...
const TYPES: readonly DamageType[] = [
  "cr",
  "cut",
  "imp",
  "pi-",
...
function rollsByLocation(): Map<HitLocation, string> {
  ...
  return new Map(
    [...runs].map(([location, list]) => [location, list.map(([a, b]) => span(a, b)).join(", ")]),
  );
}
...
const CRIPPLING_DIVISOR: Record<string, number | null> = {
  none: null,
  limb: 2,
  extremity: 3,
  eye: 10,
...
function locationNotes(location: HitLocation, t: BuildContext["t"]): string {
  ...
  if (location === "vitals")
    notes.push(
      t(`${K}.HitLocation.TightBeam`, {
        value: times(woundingModifierAt("burn", "vitals", { tightBeam: true })),
      }),
    );
...
function addonRow(added: AddonHitLocation, { t, moduleTitle }: BuildContext): GmRow {
  ...
  const notes = [
    added.damageTypes.length
      ...
      : "",
    added.arcs
      ? t(`${K}.HitLocation.Arcs`, {
          arcs: added.arcs.map((arc) => t(`${K}.Arc.${arc}`)).join(", "),
        })
      : "",
  ]
    .filter(Boolean)
...
/** Every wound topic's part, by id: each is a card of its own on the tab. */
function woundParts({ t }: BuildContext): GmPart[] {
  const p = (part: string, data?: Record<string, string | number>) =>
    t(partKey("wounds", part), data);
  // A 20-point crushing blow against ST 12 shows the arithmetic: ST-2 is 10, so two yards.
  ...
  return [
    {
      ...
      content: {
        ...
        items: [
          {
            term: p("ShockPerHp"),
            text: p("ShockPerHpText", {
              value: signed(shockPenalty(1)),
              max: signed(MAX_SHOCK_PENALTY),
            }),
          },
    ...
    },
    {
      ...
      content: {
        ...
        items: [
          {
            term: p("KnockbackDistance"),
            text: p("KnockbackDistanceText", { damage: 20, st: 12, yards: shove.yards }),
          },
          { term: p("KnockbackCutting"), text: p("KnockbackCuttingText") },
          {
            term: p("KnockbackStanding"),
            text: p("KnockbackStandingText", { value: signed(shove.fallRollPenalty || -1) }),
          },
        ],
      },
    },
    ...
    },
    {
      ...
      content: {
        ...
        rows: [
          {
            cells: [
              p("KnockdownElsewhere"),
              signed(knockdownModifier({ majorWound: true, hitLocation: "torso" })),
            ],
          },
          {
            cells: [
              p("KnockdownFace"),
              signed(knockdownModifier({ majorWound: true, hitLocation: "face" })),
            ],
          },
          {
            cells: [
              p("KnockdownSkull"),
              signed(knockdownModifier({ majorWound: true, hitLocation: "skull" })),
            ],
          },
          {
            cells: [
              p("KnockdownHighPain"),
              signed(traitEffects([{ name: "High Pain Threshold" }]).knockdown),
            ],
          },
          {
            cells: [
              p("KnockdownLowPain"),
              signed(traitEffects([{ name: "Low Pain Threshold" }]).knockdown),
            ],
          },
        ],
      },
    ...
    },
    {
      ...
      content: {
        ...
        items: [
          {
            term: p("StunDefense"),
            text: p("StunDefenseText", { value: signed(STUN_DEFENSE_PENALTY) }),
          },
    ...
    },
    {
      ...
      content: {
        ...
        items: [
          ...
          {
            term: p("CripplingLasting"),
            text: p("CripplingLastingText", {
              tl7: cripplingRelief(7),
              tl6: cripplingRelief(6),
              tl5: cripplingRelief(5),
            }),
          },
        ],
      },
    },
    {
      ...
      content: {
        ...
        items: [
          ...
          {
            term: p("MortalTrauma"),
            text: p("MortalTraumaText", { minutes: TRAUMA_MAINTENANCE_MINUTES }),
          },
        ],
      },
    },
    {
      ...
      content: {
        ...
        items: [
          ...
          {
            term: p("BleedingRoll"),
            text: p("BleedingRollText", { value: signed(bleedingModifier(5)) }),
          },
          {
            term: p("BleedingLoss"),
            text: p("BleedingLossText", { hp: BLEEDING_HP, critical: BLEEDING_CRITICAL_HP }),
          },
          {
            term: p("BleedingStops"),
            text: p("BleedingStopsText", { minutes: MINUTES_TO_STOP_BLEEDING }),
          },
        ],
      },
    },
...
/** One wound topic as a card: its part, without the heading the card's title already gives. */
function woundContent(context: BuildContext, id: string): GmSectionContent {
  const part = woundParts(context).find((p) => p.id === id)!;
  return { parts: [{ content: part.content, ...(part.notes ? { notes: part.notes } : {}) }] };
}
...
export const WOUNDS_SECTIONS: readonly GmSectionDef[] = [
  section({
    ...
    build: (context) => {
      ...
      for (const location of HIT_LOCATION_ORDER) {
        ...
        for (const own of added.filter((a) => a.parent === location))
          rows.push(addonRow(own, context));
      }
      return {
        parts: [
          {
  ...
  section({
    ...
    tab: "wounds",
    cite: "p. B419",
    build: (context) => woundContent(context, "shock"),
  }),
  section({
    id: "knockback",
    tab: "wounds",
    cite: "p. B378",
    build: (context) => woundContent(context, "knockback"),
  }),
  section({
    id: "majorWound",
    tab: "wounds",
    cite: "p. B420",
    build: (context) => woundContent(context, "majorWound"),
  }),
  section({
    id: "knockdownStunning",
    tab: "wounds",
    cite: "p. B420",
    build: (context) => woundContent(context, "knockdownStunning"),
  }),
  section({
    id: "effectsOfStun",
    tab: "wounds",
    cite: "p. B420",
    build: (context) => woundContent(context, "effectsOfStun"),
  }),
  section({
    id: "cripplingInjury",
    tab: "wounds",
    cite: "pp. B420-422",
    build: (context) => woundContent(context, "cripplingInjury"),
  }),
  section({
    id: "mortalWounds",
    tab: "wounds",
    cite: "p. B423",
    build: (context) => woundContent(context, "mortalWounds"),
  }),
  section({
    id: "bleeding",
    tab: "wounds",
    cite: "p. B420",
    build: (context) => woundContent(context, "bleeding"),
  }),
];
