...
import { OPPORTUNITY_LINE_PENALTY, bulkPenalty, opportunityFirePenalty } from "../../../rules/attack-options.js";
import { COMPLETELY_CONCEALED_PENALTY, HALF_EXPOSED_PENALTY, SHOOT_THROUGH_PENALTY } from "../../../rules/cover.js";
...
const WOUNDING_ORDER: readonly DamageType[] = ["pi-", "burn", "cor", "cr", "fat", "pi", "tox", "cut", "pi+", "imp", "pi++"];
...
export const RANGED_SECTIONS: readonly GmSectionDef[] = [
      ...
      return {
        parts: [{
          content: {
            kind: "rules",
            items: [
              m("Aim", { extra: signed(aimedThree.extraTurns), turns: MAX_EXTRA_AIM_TURNS + 1, braced: signed(BRACED_BONUS) }),
  ...
  section({
    ...
    build: ({ t }) => {
      const rows: GmRow[] = [
        ...SMALL_INCHES.map((length) => ({ cells: [signed(sizeModifier(length / 36)), inches(length), "0"] })),
        ...yardRows(100_000).map((yards) => ({ cells: [signed(sizeModifier(yards)), yardsText(yards), signed(speedRangeModifier(yards))] })),
      ];
      return {
        parts: [{
          content: {
            kind: "table",
            columns: [t(`${K}.Column.SizeModifier`), t(`${K}.Column.Length`), t(`${K}.Column.SpeedRange`)],
            rows,
          },
        }],
  ...
  section({
    ...
    build: ({ t }) => ({
      parts: [{
        content: {
          kind: "table",
          columns: [t(`${K}.Column.DamageType`), t(`${K}.Column.Abbreviation`), t(`${K}.Column.Modifier`)],
          rows: WOUNDING_ORDER.map((type) => ({ cells: [t(`${K}.DamageType.${type}`), t(`GWORLD.DamageType.${type}`), times(WOUNDING_MODIFIERS[type])] })),
        },
      }],
      ...
      parts: [{
        content: {
          ...
          rows: FIRST_AID_TABLE.map((row, index) => {
            ...
            return { cells: [span(row.fromTl, next ? next.fromTl - 1 : null), t(`${K}.Minutes`, { value: row.minutes }), dice(row.restored)] };
          }),
        },
      }],
      notes: [t(`${K}.Section.firstAid.Note`, { bandage: BANDAGING_HP, fumble: FIRST_AID_CRITICAL_FAILURE_HP })],
    }),
  }),
  section({
    id: "naturalRecovery",
      ...
      return {
        parts: [{
          content: {
            ...
            rows: [
              { key: above.kind, cells: [t(`${K}.Waking.Positive`), t(`${K}.Waking.Automatic`, { minutes: above.minutes })] },
              { key: zero.kind, cells: [t(`${K}.Waking.Zero`), t(`${K}.Waking.Hourly`, { minutes: zero.minutes })] },
              { key: deep.kind, cells: [t(`${K}.Waking.MinusHp`), t(`${K}.Waking.Mortal`, { hours: deep.minutes / 60 })] },
            ],
          },
        }],
      };
